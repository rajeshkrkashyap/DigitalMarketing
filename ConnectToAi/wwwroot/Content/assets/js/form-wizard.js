var FormWizard = function () {
    ...
    return {
        init: function () {
            initWizard();
            if ($('a[href="/Account/Logout"]').eq(0).length > 0) {
                console.log("Your are looged in");
                if ($('div[title="MakePayment1"]').eq(0).length > 0) {
                    console.log("goToStep 2");
                    wizardContent.smartWizard("goToStep", 2);
                }
                if ($('div[title="MakePayment"]').eq(0).length > 0) {
                    console.log("goToStep 3");
                    wizardContent.smartWizard("goToStep", 3);
                }
                if ($('div[title="Success1"]').eq(0).length > 0) {
                    console.log("goToStep 3");
                    wizardContent.smartWizard("goToStep", 3);
                }
                if ($('div[title="Success"]').eq(0).length > 0) {
                    console.log("goToStep 4");
                    wizardContent.smartWizard("goToStep", 4);
                }
            }
            else {

                if ($('div[title="UserAccount"]').eq(0).length > 0) {
                    console.log("goToStep 2");
                    wizardContent.smartWizard("goToStep", 2);
                }
